...
import {
  Eye, Users, AlertTriangle, HelpCircle, CalendarDays, Clock, MapPin,
...
export default function EventDetails() {
  ...
  return (
    <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }}
      ...
      {/* ══ HERO ══ */}
      <div ref={heroRef}
        className={`relative w-full overflow-hidden bg-gradient-to-br ${BG_GRADIENT[ev.bg] || BG_GRADIENT.bg1}`}
        style={{ minHeight: 320 }}>
      ...
      <div className="px-4 md:px-10 -mt-4 mb-6 relative z-10 md:max-w-[900px] md:mx-auto">
        <div className="flex gap-3 overflow-x-auto no-scrollbar pb-1">
          <StatPill icon={<Eye size={18} strokeWidth={1.8} className="text-text-3" />} value={(ev.viewCount||0).toLocaleString('en-IN')} label="Views" />
          <StatPill icon={<Users size={18} strokeWidth={1.8} className="text-text-3" />} value={`${(ev.registrationCount||0).toLocaleString('en-IN')}+`} label="Registered" />
          {ev.deadlineDays > 0 && ev.deadlineDays <= 12 && (
